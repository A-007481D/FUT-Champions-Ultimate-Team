...
function openEditPM() {
  const editModal = document.getElementById("editPlayer-modal");
  editModal.style.display = "flex";

  let name = selectedP.querySelector('.player-NAME');

  let player = playerArray.filter(player => player.name == name.textContent)[0];
  editModal.querySelector('#playerName').value = player.name;
  editModal.querySelector('#playerNationality').value = player.nationality;
  ...
  editModal.querySelector('#addEdited-player').onclick = function () {
    ...
    for (let i = 0; i < playerArray.length; i++) {
      if (playerArray[i].name == player.name) {
        ...
        playerArray[i].nationality = `https://flagsapi.com/${editModal.querySelector('#playerNationality').value.toUpperCase()}/flat/64.png`
        playerArray[i].club = `https://raw.githubusercontent.com/A-007481D/FB-Clubs-API/refs/heads/main/teams/${editModal.querySelector('#ClubLogo').value.toUpperCase()}.png`


        name.textContent = playerArray[i].name;
        selectedP.querySelector('.player-flag').src = playerArray[i].nationality;
        editModal.querySelector('#ClubLogo').value = player.club;
        selectedP.querySelector('.player-club').src = playerArray[i].club;
      }
    }
    closeEditModal();
  ...
  playerArray.forEach((player) => {
    ...
    playerCard.innerHTML = `
      ...
      player.name
        ...
        <button class="edit-btn card-btns bg-green-500 hover:bg-green-700 group-hover:opacity-100 opacity-0 text-white font-bold py-1.5 px-3 rounded">
...
<svg class="w-4 " fill="#fff" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448  512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>
      ...
      playerCard.querySelector('.edit-btn').addEventListener('click', function (e) {
        e.stopPropagation();
        selectedP = this.parentElement.parentElement;
        openEditPM();
      })
